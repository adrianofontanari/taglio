...
1. `AGENTS.md` (repository root) — hard rules.
...
Run the project's verify gate: `npm test && npm run compile`.
...
Once the change is on `main` (fast-forwarded):

- `git mv plan/todo/NNNN-<slug>.md plan/done/<YYYY-MM-DD>-<slug>.md`.
- Amend the moved file with a "Shipped at HEAD `<sha>`" footer.
